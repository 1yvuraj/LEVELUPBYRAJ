...
public class leetcodeQuestion {
    ...
    //47
    public static int permute2(int[] coins, int idx,List<List<Integer>> ans,ArrayList<Integer> smallAns) {
        if (idx == coins.length) {
        ...
        for (int i = 0; i < coins.length; i++) {
            if (prev!=coins[i] && coins[i] >-11 ) {
                ...
                coins[i] = -11;
                smallAns.add(val);
                count += permute2(coins,idx+1,ans,smallAns);
                coins[i] = val;
                smallAns.remove(smallAns.size()-1);
                prev=coins[i];
            }
    ...
    // 40
    public static int combinationSum2(int[] arr, int tar, int idx, List<List<Integer>> ans, List<Integer> smallAns) {
        if (tar == 0) {
            List<Integer> base = new ArrayList<>(smallAns);
            ans.add(base);
            return 1;
